...
func BackTrackAlgorithm(tetrominoes [][4][4]string, n int) bool {
	...
	for i := 0; i < len(mySquare); i++ {
		for j := 0; j < len(mySquare); j++ {
			if ValidInsertion(i, j, tetrominoes[n]) { // check if we can place current tetrominoe on the board anywhere
				Insert(i, j, tetrominoes[n]) // if we can place it at this location, check if we can place another piece
				if BackTrackAlgorithm(tetrominoes, n+1) {
					return true
				}
...
func GeniusSolver(tetrominoes [][4][4]string) [][]string {
	// initial board starts with dimension 4*4, if we can't place all tetrominoes
	// increase size by 1 and initialize board
	l := int(math.Ceil(math.Sqrt(float64(4 * len(tetrominoes)))))
	// if l < 4 {
	// 	l = 4
	// }
	mySquare = SquareCreator(l)
	for !BackTrackAlgorithm(tetrominoes, 0) {
		l++
		mySquare = SquareCreator(l)
	}
